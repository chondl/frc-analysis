...
(local-only). Plan: `../docs/superpowers/plans/2026-07-14-team-longevity-analysis.md`.
...
2. **`compute.js`** — pure derivation of every series (survival, flows, age-standardized
   ...
   `compute.test.mjs`; inlined verbatim into the tool by the build (its `export` keywords are
   stripped so the browser calls the exact same functions the tests exercise).
...
(gitignored, not ephemeral `/tmp`). TBA key is read from `~/thebluealliance_api_key.txt`.
...
Regional/state slicing and the EPA competitiveness ("health") cut are deliberately excluded from
this first cut; the data model does not pre-build for them.
